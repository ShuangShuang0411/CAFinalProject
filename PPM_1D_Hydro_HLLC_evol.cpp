...
//    for (int j=1;j<N-2;j++)   a_half[j] = 7.0/12.0*(a[j]+a[j+1])-1.0/12.0*(a[j+2]+a[j-1]);
//    a_half[0] = 7.0/12.0*(a[0]+a[1])-1.0/12.0*(a[2]+a[0]);   // because this is outflow BC
//    a_half[N-1] = 7.0/12.0*(a[N-1]+a[N-1])-1.0/12.0*(a[N-1]+a[N-2]);
//    a_half[N-2] = 7.0/12.0*(a[N-2]+a[N-1])-1.0/12.0*(a[N-1]+a[N-3]);
...
void correction_HLL (int LeftRight, double dt, double *lambda_p, double *lambda_n, double *lambda_0, double *cs, double **W, double **delm_W, double **value){
...
//  Correction for HLL solver 
    ...
    for (int i=0;i<3;i++)   get_aLR(W[i], tempL, tempR, delm_W[i]);

    correction_HLL (0, dt, lambda_p, lambda_n, lambda_0, cs, W, delm_W, corr_L);
    correction_HLL (1, dt, lambda_p, lambda_n, lambda_0, cs, W, delm_W, corr_R);
...
int main(int argc, const char * argv[]) {
    ...
    //save data into file
    FILE * data_ptr;
    data_ptr = fopen("./bin/PPM_data_evol.txt", "w");
...
*/
            ...
            Conserved2Primitive(U, W_prime);
            for (int i=0;i<3;i++){
                get_aLR(W_prime[i], W_prime_L[i], W_prime_R[i], temp);
//                for (int j=0;j<N;j++)   W_prime_L[i][j] = 6*W_prime[i][j]-2*W_prime_L[i][j]-3*W_prime_R[i][j];
            }
            Primitive2Conserved(W_prime_L, U_L);
            Primitive2Conserved(W_prime_R, U_R);
